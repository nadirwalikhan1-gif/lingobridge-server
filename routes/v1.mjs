...
// FIX: REMOVED — POST /wallet/top-up. This route let any authenticated
// client credit their OWN wallet by an arbitrary `amount`, calling
// credit_wallet_topup directly with zero payment verification: no
// LemonSqueezy checkout, no signed webhook, nothing. Confirmed via a full
// grep of the client codebase that no frontend code calls this route at
// all — it was live, reachable by anyone with a valid login token (e.g.
// via curl/Postman), and completely unused by the actual app. The real,
// safe flow is: POST /create-checkout (validates amount/currency, creates
// a LemonSqueezy checkout URL) -> user pays on LemonSqueezy -> LemonSqueezy
// sends a SIGNATURE-VERIFIED webhook to POST /webhook/lemonsqueezy ->
// processOrderCreated() credits the wallet only then. This route bypassed
// every one of those steps. If a manual/admin credit feature is wanted in
// the future, it should go through admin.mjs's existing requireAdmin-gated
// creditWallet() call (see the dispute-refund flow there for the pattern),
// never as a public client-callable endpoint accepting a raw amount.
